...
## 6e. Run `part-11-product-collections.sql` — product placement tags

Same SQL Editor → **New query** → paste the entire contents of
`supabase/part-11-product-collections.sql` → **Run**.

This adds one column, `products.collections`, which stores which homepage /
Shop page categories a product is ticked into (Admin → Add / Edit product →
**Where this product appears**). Safe to re-run, changes no existing data.

**Run it before starting the app version that includes this feature** — the
storefront and the admin form both read/write this column, so the product
queries fail until it exists. (If you started from a fresh `schema.sql` that
already includes the column, you can skip this step.)

## 6f. Run `part-12-product-colors.sql` — multi-color products

Same SQL Editor → **New query** → paste the entire contents of
`supabase/part-12-product-colors.sql` → **Run**.

This adds one new table, `product_colors`, which lets a single product
declare a list of colors it's available in (Admin → Add / Edit product →
**Colors**) without needing a separate photo shoot per color — the
storefront recolors the one uploaded photo set on the fly (see
`src/components/product/ProductGallery.jsx`). Safe to re-run, changes no
existing data.

**Run it before starting the app version that includes this feature** — the
storefront and the admin form both read/write this table, so the product
queries fail until it exists. A product with no rows here is unaffected —
it just falls back to the single color already parsed from its name.
