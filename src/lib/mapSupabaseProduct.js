...
export function mapSupabaseProduct(row) {
  ...
  const colors = (row.product_colors || [])
    .map((c) => ({ id: c.id, name: c.name, hex: c.hex_code, sortOrder: c.sort_order ?? 0 }))
  ...
  return {
    ...
    variants,
    images,
    // Admin-configured color options (Part 12) — index 0 is treated by the
    // storefront as "matches the uploaded photos as-is"; any other entry is
    // simulated by recoloring those same photos (see productColor.js and
    // ProductGallery.jsx). Empty when a product hasn't had any colors set,
    // in which case the storefront falls back to the legacy name-parsed
    // single color.
    colors,
    category: row.category || '',
    tags: row.tags || [],
    // Store placement — homepage/Shop category slugs (see src/data/collections.js).
